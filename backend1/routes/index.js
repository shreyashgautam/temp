...
function safeActorId(input) {
  const raw = String(input || 'anonymous').trim() || 'anonymous';
  const cleaned = raw.replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 64);
  return cleaned || 'anonymous';
}

function chatSessionsFile(actorId) {
  ensureChatDir();
  return path.join(CHAT_DIR, `sessions_${safeActorId(actorId)}.json`);
}

function readChatSessionsFromFile(actorId) {
  try {
